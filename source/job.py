...
q = HotQueue("queue", host = 'IP_ADDRESS_OF_REDIS', port = 6379, db=1)
rd = StrictRedis(host = 'SAME_IP_AS_IN_LINE_ABOVE', port = 6379, db=0)
...
def add_job(start, end, status="submitted"):
    ...
    _save_job(_gnerate_job_key(jid), job_dict)
    _queue_job(jid)
    return job_dict
